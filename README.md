...
- 团队击杀分 + 经济系统（击杀/连杀奖励 + 连死补偿）
- 击杀播报 KillFeed（按队伍着色）、计分板（Tab）
- CS 风格购买菜单（武器/护甲/手雷/医疗）、自动复活 + 随机出生点
...
├── s1mpleFpsPvPGameMode    — 多人 PvP 模式（击杀/经济/判胜）
├── ALobbyGameMode          — 大厅模式（准备/选英雄/分队/开局）
├── s1mpleFpsGameState      — 比赛状态（复制：计时/加时/团队分/KillFeed）
